  ...
  getKeywords: function(input) {
    ...
    for (var x = 0; x < lists.articles.length; x++) {
      for (var y = 0; y < parse.length; y++) {
        if (parse[y].toLowerCase() === lists.articles[x] || parse[y].length <= 2) {
          parse[y] = "";
        }
      }
    }
      ...
      for (var j = 0; j < parse.length; j++) {
        if (word === parse[j] && word !== "") {
          ...
          if (parse.length > 1000 && count >= 5) {
              results[word] = word;
          } else if ((parse.length < 1000 && parse.length > 850) && count >= 4) {
            results[word] = word;
          } else if ((parse.length < 850 && parse.length > 600) && count >= 3) {
            results[word] = word;
          } else if (parse.length < 600 && count >= 2) {
            results[word] = word;
          }
        }
      }
